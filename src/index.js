// сюда вставить объект с описанием карты из dev tools
...
// api ключ не мой, но его можно взять с официального сайта 2gis!
// открываем https://docs.2gis.com/ru/mapgl/overview. Дальше
// открываем dev tools, находим запрос по имени mapgl-js-api,
  ...
  return geometries.orderedGeometries.map((geom) => {
    ...
    if (geom.geometry.type === "Polygon") {
      ...
      newPolygon.on("click", (e) => {
        ...
        map.on("click", () => {
          popups.forEach((pp) => {
            pp.getContent().style.display = "none";
          });
        });
      });
      return {
